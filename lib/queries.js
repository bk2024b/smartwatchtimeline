...
export const getPublishedGuides = unstable_cache(async () => {
  const supabase = getSupabase();
  const { data, error } = await supabase.from('guides').select('slug, priority, category, icon, title, description, intro, sections, faq, filter, sort, published_at').eq('status', 'published').order('priority', { ascending: false });
  if (error) throw error;
  return data || [];
