...
export async function generateMetadata({ params }) {
  ...
  if (!brand) return {};
  return {
    title: `${brand.name} Smartwatches | SmartwatchTimeline`,
    description: `Every ${brand.name} smartwatch in our database, with full specs and price history.`,
    ...canonicalFor(`/brands/${params.slug}`),
  };
}
  ...
  return (
    <>
      ...
      <article className="max-w-6xl mx-auto">
        <div className="font-mono text-xs text-accent uppercase mb-3">Brand</div>
        <h1 className="font-display font-bold text-[38px] sm:text-[54px] leading-tight mb-4">{brand.name} Smartwatches</h1>
        <p className="text-dim text-[15px] sm:text-[17px] leading-7 max-w-3xl">{brandWatches.length} models tracked.</p>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mt-10">
          {brandWatches.map((watch) => (
            <ProductCard key={watch.id} watch={watch} brand={brand} productLinks={linksByWatch.get(watch.id) || []} />
          ))}
        </div>
      </article>
      <Footer />
