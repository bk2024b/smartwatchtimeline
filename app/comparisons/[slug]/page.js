...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    description: `Detailed comparison of ${a.name} and ${b.name}: battery, display, health sensors, connectivity and price.`,
...
export default async function ComparisonPage({ params }) {
  ...
  const scoreA = scoreWatch(a, b); const scoreB = scoreWatch(b, a);
  const recommendation = scoreA === scoreB ? 'It is a close match — the better choice depends on the features that matter most to you.' : `${scoreA > scoreB ? a.name : b.name} wins more of the measurable comparison points.`;
  ...
  return <>
    ...
    <article className="max-w-5xl mx-auto">
      ...
      <section className="grid md:grid-cols-[1fr_auto_1fr] gap-4 items-stretch mb-8">
        {[{ w: a, score: scoreA, other: scoreB }, { w: b, score: scoreB, other: scoreA }].map(({ w, score, other }, i) => <div key={w.id} className={`bg-panel border rounded-2xl p-5 ${score > other ? 'border-accent shadow-[0_0_40px_rgba(34,208,122,0.07)]' : 'border-line'}`}><div className="font-mono text-[9px] text-accent uppercase tracking-wider">{i === 0 ? 'Model 01' : 'Model 02'}</div><div className="font-mono text-[10px] text-dim uppercase mt-2">{brandMap.get(w.brand_id)?.name || w.brand_id}{w.gamme ? ` · ${w.gamme}` : ''}</div><h2 className="font-display font-bold text-xl mt-1">{w.name}</h2><div className="grid grid-cols-3 gap-2 mt-5 font-mono text-[10px]"><div><span className="text-dim block">Price</span>{w.price != null ? `$${Number(w.price).toFixed(0)}` : '—'}</div><div><span className="text-dim block">Battery</span>{w.battery_life_h ? `${w.battery_life_h}h` : '—'}</div><div><span className="text-dim block">Weight</span>{w.weight_g ? `${w.weight_g}g` : '—'}</div></div><Link href={`/smartwatches/${w.id}`} className="text-accent font-mono text-[10px] uppercase mt-5 inline-block">View full specs →</Link></div>)}
