...
function hasFeature(watch, field) {
  const value = watch[field];
  return value === true || value === 1 || value === 'true' || value === '1' || (typeof value === 'string' && value.trim().toLowerCase() === 'yes');
}

export async function generateMetadata() {
  return { title: 'Smartwatch Insights | SmartwatchTimeline', description: 'Explore smartwatch data trends across price, battery, weight, sensors, connectivity and design.', ...canonicalFor('/insights') };
}
...
export default async function InsightsPage() {
  ...
  const displaySizes = watches.map((w) => number(w.screen_size_mm)).filter((v) => v !== null);
  const avgPrice = average(priced), avgBattery = average(batteries), avgWeight = average(weights), avgDisplay = average(displaySizes);
  ...
  const latest = watches.filter((w) => new Date(w.release_date).getFullYear() === latestYear);
