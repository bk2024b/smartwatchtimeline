...
export default function EditBrandPage() {
  ...
  async function save(e) { e.preventDefault(); setSaving(true); setError(''); const { error: e2 } = await supabase.from('brands').update({ name: form.name.trim(), color: form.color.trim() }).eq('id', id); if (e2) setError(e2.message); else { await revalidateAdminResource('brand', id); router.push('/admin/brands'); } setSaving(false); }
  async function remove() { if (!confirm(`Delete ${form.name}?`)) return; const { error: e } = await supabase.from('brands').delete().eq('id', id); if (e) setError(e.message); else { await revalidateAdminResource('brand', id); router.push('/admin/brands'); } }
